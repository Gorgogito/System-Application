...
public class TaskBoardController : ControllerBase
{
    ...
    /// <summary>Reiniciar mes: mover todas las tarjetas a Pendiente excepto las Suspendidas</summary>
    [HttpPost("ResetMonth")]
    public async Task<IActionResult> ResetMonth()
    {
        var result = await _service.ResetMonthAsync(CurrentUser);
        return Ok(ApiResponse<ResetMonthResult>.Ok(result,
            $"Reinicio completado: {result.MovedCount} tarjeta(s) movida(s) a Pendiente, " +
            $"{result.SkippedCount} permanecieron en Suspendido."));
    }
}
